...
public class NotifyTest extends CamelTestSupport {
	...
	public void testNotifyFrom() throws Exception {
		// use from to indicate it should only be messages originating from
		// the given endpoint
		NotifyBuilder notify = new NotifyBuilder(context)
			.from("seda:order").whenDone(1).create();

		template.sendBody("seda:quote", "Camel rocks");
		template.sendBody("seda:order", "123,2020-05-07'T'15:47:59,4444,5555");
		...
		assertEquals("OK,123,2020-05-07'T'15:47:59,4444,5555", confirm.getExchanges().get(0).getIn().getBody());
	}
	...
	public void testNotifyWhenAnyDoneMatches() throws Exception {
		...
		NotifyBuilder notify = new NotifyBuilder(context)
				.from("seda:order")
				.whenAnyDoneMatches(body().isEqualTo("OK,123,2020-05-07'T'15:48:00,2222,3333"))
				.create();
		
		// send in 2 messages. Its the 2nd message we want to test
		template.sendBody("seda:order", "123,2020-05-07'T'15:47:59,4444,5555");
        template.sendBody("seda:order", "123,2020-05-07'T'15:48:00,2222,3333");
        ...
        assertEquals("OK,123,2020-05-07'T'15:48:00,2222,3333", confirm.getExchanges().get(1).getIn().getBody());
	}
